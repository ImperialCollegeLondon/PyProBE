...
class dQdV_OCV_fit(BaseMethod):
    ...
    def __init__(
        ...
        self.dQdV = np.gradient(self.SOC, self.voltage)
        ...
        self.fitted_OCV = self.make_result(
            {
                ...
                "SOC": self.SOC,
                "Input Voltage": self.voltage,
                "Fitted Voltage [V]": OCV,
                "Input dSOCdV [1/V]": self.dQdV,
                "Fitted dSOCdV [1/V]": np.gradient(self.SOC, OCV),
            }
        )
    ...
    def cost_function(self, params: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        dQdV = np.gradient(self.SOC, modelled_OCV)
        # print(dVdQ)
        return np.sum((dQdV - self.dQdV) ** 2)
