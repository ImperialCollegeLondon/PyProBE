    ...
    dataframe = pl.DataFrame(
        {
            ...
            "Step Index / 1": steps,
            "Step Count / 1": list(range(len(steps))),
            "Current / A": steps,
            "Voltage / V": steps,
            "Capacity / Ah": steps,
        },
...
def test_cycle_generic(generic_experiment):
    ...
    assert filters._cycle(next_cycle, 0).data[
        "Test Time / s"
    ].unique().to_list() == list(
        range(26, 31),
    )
    assert filters._cycle(next_cycle, 3).data[
        "Test Time / s"
    ].unique().to_list() == list(
        range(41, 46),
    )
    assert filters._cycle(next_cycle, -1).data[
        "Test Time / s"
    ].unique().to_list() == list(
        range(46, 52),
    )
