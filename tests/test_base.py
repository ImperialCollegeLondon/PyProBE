...
def test_zero_capacity(base_instance):
    ...
    assert base_instance.raw_data["Capacity (Ah)"][0] == 0


def test_RawData(base_instance):
    assert base_instance._raw_data is None
    procedure = base_instance.raw_data
...
def test_mA_units(base_instance):
    ...
    assert_series_equal(base_instance.raw_data["Current (mA)"], 
                        1000*base_instance.raw_data["Current (A)"],
                        check_names=False)
    assert_series_equal(base_instance.raw_data["Capacity (mAh)"], 
                        1000*base_instance.raw_data["Capacity (Ah)"],
                        check_names=False)
    
def test_capacity_throughput(BreakinCycles_fixture):
    step_capacity = BreakinCycles_fixture.cycle(0).charge(0).capacity
    step_capacity_throughput = BreakinCycles_fixture.cycle(0).charge(0).raw_data["Capacity Throughput (Ah)"].tail(1).to_list()[0]
