...
def test_get_resistances(Pulsing_fixture):
    ...
    assert resistances.get("R0 / Ohm")[0] == (4.1558 - 4.1919) / -0.0199936
    assert resistances.get("R_10s / Ohm")[0] == (4.1337 - 4.1919) / -0.0199936
...
def test_get_ocv_curve(Pulsing_fixture):
    ...
    assert result.columns.names == Pulsing_fixture.columns.names
    assert np.allclose(result.get(BDF.VOLTAGE_VOLT.name), expected_ocv_points)
